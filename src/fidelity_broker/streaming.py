"""Fidelity WebSocket Streaming (PRD-156).

Real-time market data streaming via Fidelity Streamer WebSocket.
...
class StreamChannel(str, Enum):
    """Available Fidelity streaming channels."""
    QUOTE = "QUOTE"
    CHART = "CHART_EQUITY"
    OPTION = "OPTION"
...
@dataclass
class StreamEvent:
    """A streaming event from Fidelity."""
...
class FidelityStreaming:
    """Manages WebSocket connection for real-time Fidelity data.
    ...
    async def start(self) -> None:
        ...
        if self._running:
            return
        self._running = True
        logger.info("Fidelity streaming started")
    ...
    async def stop(self) -> None:
        ...
        self._tasks.clear()
        logger.info("Fidelity streaming stopped")
